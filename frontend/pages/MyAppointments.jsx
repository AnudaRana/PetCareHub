...
const AppointmentDetails = ({ appointment, showUpdatedTag = false }) => (
  <>
    ...
    <p>
      ...
      {showUpdatedTag && appointment.isUpdated && (
...
const MyAppointments = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const mapped = (Array.isArray(res.data) ? res.data : []).map((appt) => ({
        ...appt,
        isUpdated: false,
      }));
  ...
  const confirmUpdate = async (e) => {
    ...
    try {
      ...
      setAppointments((prev) =>
        prev.map((appt) =>
          appt.id === selectedAppointment.id ? { ...res.data, isUpdated: true } : appt
        )
      );
  ...
  const confirmCancel = async (e) => {
    ...
    try {
      ...
      setAppointments((prev) =>
        prev.map((appt) =>
          appt.id === appointmentId ? { ...res.data, isUpdated: false } : appt
        )
      );
