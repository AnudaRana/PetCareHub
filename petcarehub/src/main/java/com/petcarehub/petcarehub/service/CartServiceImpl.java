...
import com.petcarehub.petcarehub.dto.CartItemDto;
...
public class CartServiceImpl implements CartService {
    ...
    public CartResponseDto getCart(Long userId) {
        ...
        for (Long pid : qtyMap.keySet()) {
            ...
            dto.imageUrl = "/api/products/" + dto.productId + "/image";
            items.add(dto);
        }
    ...
    public CartResponseDto updateQuantity(Long userId, Long productId, Integer quantity) {
        ...
        if (existing.isEmpty()) throw new EntityNotFoundException("Cart item not found");

        Cart keep = existing.get(0);
        keep.setQuantity(quantity);
        cartRepository.save(keep);

        for (int i = 1; i < existing.size(); i++) cartRepository.delete(existing.get(i));
